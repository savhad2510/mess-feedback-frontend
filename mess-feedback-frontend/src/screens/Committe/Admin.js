...
function Committe() {
  ...
  return (
    <div className="admin-container">
      <h2>Committe portal</h2>
      <div className="button-container">
        <button onClick={() => handleAddClick(true)}>Add Resident officer</button>
        <button onClick={() => handleAddClick(false)}>Add campus director</button>
      </div>
      {showForm && (
        <div className="modal">
      ...
      <div className="dropdown">
        <button onClick={() => setShowSupervisors(!showSupervisors)}>
          Resident officer {showSupervisors ? '▲' : '▼'}
        </button>
        {showSupervisors && (
          <div>
            {supervisors.map(s => (
